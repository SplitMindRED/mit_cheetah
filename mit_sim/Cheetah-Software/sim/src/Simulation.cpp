  ...
  : _simParams(params), _userParams(userParams), _tau(12)
{
  ...
  else if (_robot == RobotType::CHEETAH_3)
  {
    ...
    for (int leg = 0; leg < 4; leg++)
    {
      ...
      _tiBoards[leg].set_link_lengths(_quadruped._abadLinkLength, _quadruped._hipLinkLength,
...
void Simulation::sendControlParameter(const std::string& name, ControlParameterValue value,
...
void Simulation::firstRun()
{
  ...
  std::cout << "start sending uer params" << std::endl;
...
void Simulation::highLevelControl()
{
  ...
  _imuSimulator->updateCheaterState(_simulator->getState(), _simulator->getDState(),
 ...
 */
void Simulation::addCollisionPlane(double mu, double resti, double height, double sizeX, double sizeY, double checkerX,
 ...
 * @param addToWindow : if true, also adds graphics for the plane
 */
void Simulation::addCollisionBox(double mu, double resti, double depth, double width, double height,
                                 const Vec3<double>& pos, const Mat3<double>& ori, bool addToWindow, bool transparent)
{
...
void Simulation::addCollisionMesh(double mu, double resti, double grid_size, const Vec3<double>& left_corner_loc,
                                  const DMat<double>& height_map, bool addToWindow, bool transparent)
{
...
void Simulation::runAtSpeed(std::function<void(std::string)> errorCallback, bool graphics)
{
  ...
  printf("[Simulator] Starting run loop (dt %f, dt-low-level %f, dt-high-level %f "
         "speed %f graphics %d)...\n",
         _simParams.dynamics_dt, _simParams.low_level_dt, _simParams.high_level_dt, _simParams.simulation_speed,
         graphics);
    ...
    if (frameTimer.getSeconds() > frameTime)
    {
      ...
      if (graphics && _window)
      {
        ...
        sprintf(_window->infoString,
                ...
                "sim-time:   %8.3f\n"
                "rate:       %8.3f\n",
                _desiredSimSpeed, freeRunTimer.getSeconds(), _currentSimTime, simRate);
        updateGraphics();
      }
  ...
  for (auto& key : keys)
  {
    ...
    else if (typeName == "mesh")
    {
      ...
      if (file_input)
      {
        ...
        if (!f_height.good())
        {
          std::cout << "file reading error: " << THIS_COM "../config/" + file_name << std::endl;
        }
        int i(0);
        int j(0);
        double tmp;

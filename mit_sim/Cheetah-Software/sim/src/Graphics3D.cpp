...
Graphics3D::Graphics3D(QWidget *parent)
  : QOpenGLWidget(parent), _animating(false), _colorArrayProgram(0), _frame(0), _v0(0, 0, 0), _freeCamFilter(1, 60, _v0)
{
 ...
 */
size_t Graphics3D::setupCheetah3(Vec4<float> color, bool useOld, bool canHide)
{
  return _drawList.addCheetah3(color, useOld, canHide);
}

/*!
 * Configure the window for displaying mini cheetah
 */
size_t Graphics3D::setupMiniCheetah(Vec4<float> color, bool useOld, bool canHide)
{
  return _drawList.addMiniCheetah(color, useOld, canHide);
}
...
void Graphics3D::updateCameraMatrix()
{
  ...
  if (_arrowsPressed[0])
    _ry -= _targetSpeed / 2.f;
  if (_arrowsPressed[1])
    _ry += _targetSpeed / 2.f;
  if (_arrowsPressed[2])
    _rx += _targetSpeed / 2.f;
  if (_arrowsPressed[3])
    _rx -= _targetSpeed / 2.f;
  if (!_rotOrig)
  {
  ...
  else
  {
    ...
    _cameraMatrix.translate(-_cameraTarget[0], -_cameraTarget[1], 0);
    cameraPos = Vec3<float>(-_cameraTarget[0], -_cameraTarget[1], 0);
  }
...
void Graphics3D::renderDrawlist()
{
  ...
  if (show_floor)
  {
    ...
    for (size_t id = 0; id < _drawList.getNumObjectsToDraw(); id++)
    {
      ...
      if (!_drawList.getCanBeHidden(id) || show_robot)
      {
        ...
        {
          _colorArrayProgram->setUniformValue(_matrixUniformColorArray, _cameraMatrix *
                                                                            _drawList.getModelKinematicTransform(id) *
  ...
  for (size_t id = 0; id < _drawList.getNumObjectsToDraw(); id++)
  {
    ...
    if (!_drawList.getCanBeHidden(id) || show_robot)
    {
      if (_drawList._instanceColor[id].useSolidColor)
      {
        _solidColorProgram->setUniformValue(_matrixUniformSolidColor, _cameraMatrix *
                                                                          _drawList.getModelKinematicTransform(id) *
                                                                          _drawList.getModelBaseTransform(id));
...
void Graphics3D::_MeshObstacleDrawing()
{
  ...
  glPushMatrix();
  glTranslatef(_drawList.getHeightMapLeftCorner()[0], _drawList.getHeightMapLeftCorner()[1],
...
void Graphics3D::_SetRGBHeight(const double &h, const double &step, const int &idx, float &r, float &g, float &b)
{
...
void Graphics3D::_BoxObstacleDrawing()
{
  ...
  for (size_t i(0); i < nBox; ++i)
  {
    ...
    _DrawBox(_drawList.getBoxInfoList()[i].depth, _drawList.getBoxInfoList()[i].width,
...
void Graphics3D::_DrawContactPoint()
{
  ...
  for (size_t i(0); i < _drawList.getTotalNumGC(); ++i)
  {
    ...
    glVertex3f(_drawList.getGCPos(i)[0], _drawList.getGCPos(i)[1], _drawList.getGCPos(i)[2]);
...
void Graphics3D::_drawArrow(ArrowVisualization &arrow)
{
  ...
  _drawArrow(arrow.base_position, arrow.direction, arrow.shaft_width, arrow.head_width, arrow.head_length);
}

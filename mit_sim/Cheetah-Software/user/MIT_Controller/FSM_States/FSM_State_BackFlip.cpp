...
template <typename T>
void FSM_State_BackFlip<T>::_SetJPosInterPts(const size_t& curr_iter, size_t max_iter, int leg, const Vec3<T>& ini,
                                             const Vec3<T>& fin)
{
  float a(0.f);
  float b(1.f);
...
FSM_StateName FSM_State_BackFlip<T>::checkTransition()
{
  ...
  switch ((int)this->_data->controlParameters->control_mode)
  {
    ...
    default:
      std::cout << "[CONTROL FSM] Bad Request: Cannot transition from " << K_BACKFLIP << " to "

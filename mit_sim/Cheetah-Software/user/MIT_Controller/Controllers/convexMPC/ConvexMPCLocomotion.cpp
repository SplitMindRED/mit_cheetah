...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data)
{
  ...
  for (int foot = 0; foot < 4; foot++)
  {
    ...
    if (swingState > 0)  // foot is in swing
    {
      ...
      //                                          hw_i->leg_controller->leg_datas[foot].qd, 0); // velocity dependent
      //                                          friction compensation todo removed
...
void ConvexMPCLocomotion::solveDenseMPC(int* mpcTable, ControlFSMData<float>& data)
{
  ...
  update_solver_settings(_parameters->jcqp_max_iter, _parameters->jcqp_rho, _parameters->jcqp_sigma,
                         _parameters->jcqp_alpha, _parameters->jcqp_terminate, _parameters->use_jcqp);

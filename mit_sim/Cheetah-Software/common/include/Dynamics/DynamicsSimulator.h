...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  DynamicsSimulator(FloatingBaseModel<T>& model,
  ...
  {
    _contact_constr->AddCollision(new CollisionBox<T>(mu, rest, depth, width, height, pos, ori));
  }
  ...
  {
    _contact_constr->AddCollision(new CollisionMesh<T>(mu, rest, grid_size, left_corner_loc, height_map));
  }

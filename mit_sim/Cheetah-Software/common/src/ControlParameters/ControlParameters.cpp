...
void ControlParameters::initializeFromIniFile(const std::string& path)
{
  ...
  {
    printf("[ERROR] Could not open ini file %s : not initializing control "
           "parameters!\n",
           path.c_str());
  ...
  if (sections.size() != 1)
  {
    printf("[ERROR] INI file %s had %ld sections (expected 1) : not initializing "
           "control parameters\n",
  ...
  {
    printf("[ERROR] INI file %s has section name %s, which cannot be used to "
           "initialize %s\n",

...
void TI_BoardControl::run_ti_board_iteration()
{
  ...
  {
    impedanceControl(_side_sign, data->q, data->dq, command.position_des, command.velocity_des, command.kp, command.kd,
                     command.force_ff, command.tau_ff, data->position, data->velocity, data->force, data->tau_des);
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      {
        data->tau_des[i] = data->tau_des[i] / std::abs(data->tau_des[i]) * command.max_torque;
      }
    }
  }
  else
  {
    float Jacobian[3][3];
    kinematics(_side_sign, data->q, data->dq, data->position, data->velocity, Jacobian);
    data->force[0] = 0;
    data->force[1] = 0;
    data->force[2] = 0;
    data->tau_des[0] = 0;
 ...
 */
void TI_BoardControl::impedanceControl(const float side_sign, const float q[3], const float dq[3],
 ...
 */
void TI_BoardControl::kinematics(const float side_sign, const float q[3], const float dq[3], float *p, float *v,
                                 float J[][3])
{

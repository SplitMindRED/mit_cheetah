...
void LordImu::get_device_info()
{
  ...
  printf("  name: %s\n"
         ...
         "  ahrs rate: %d Hz\n"
         "  filter rate: %d Hz\n",
         deviceInfo.modelName.c_str(), deviceInfo.modelNumber.c_str(), deviceInfo.serialNumber.c_str(),
         deviceInfo.lotNumber.c_str(), deviceInfo.deviceOptions.c_str(), deviceInfo.firmwareVersion,
         deviceInfo.ahrs_base_rate, deviceInfo.filter_rate);
}
...
void LordImu::setup_streaming()
{
  ...
  // callbacks
  if (mip_interface_add_descriptor_set_callback(&device_interface, MIP_FILTER_DATA_SET, nullptr, filter_callback) !=
      MIP_INTERFACE_OK)
  {
    throw std::runtime_error("failed to set IMU filter callback");
  }

  if (mip_interface_add_descriptor_set_callback(&device_interface, MIP_AHRS_DATA_SET, nullptr, ahrs_callback) !=
      MIP_INTERFACE_OK)
  {
  ...
  u8 num_entries = 2;
  while (mip_3dm_cmd_ahrs_message_format(&device_interface, MIP_FUNCTION_SELECTOR_WRITE, &num_entries, data_types,
                                         data_downsampling) != MIP_INTERFACE_OK)
  {
    printf("fail\n");
  }
  ...
  while (mip_3dm_cmd_filter_message_format(&device_interface, MIP_FUNCTION_SELECTOR_WRITE, &num_entries, data_types,
                                           data_downsampling) != MIP_INTERFACE_OK)
  {
    printf("fail\n");
  }
...
void LordImu::print_packet_stats()
{
  printf("-- IMU Packets --\n"
         ...
         "timeout: %d\n"
         "unknown: %d\n",
         good_packets, invalid_packets, timeout_packets, unknown_packets);
}

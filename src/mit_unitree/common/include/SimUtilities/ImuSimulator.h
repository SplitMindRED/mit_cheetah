...
public:
  explicit ImuSimulator(SimulatorControlParameters& simSettings, u64 seed = 0)
    ...
    , _vectornavAccelerometerDistribution(-simSettings.vectornav_imu_accelerometer_noise,
  ...
  void updateVectornav(const FBModelState<T>& robotState, const FBModelStateDerivative<T>& robotStateD,
                       VectorNavData* data);
  ...
  void updateCheaterState(const FBModelState<T>& robotState, const FBModelStateDerivative<T>& robotStateD,

                                          ...
                                          std::uniform_real_distribution<float> &dist, const RotMat<float> &R_body)
{
  ...
  // acceleration
  acc +=
      spatial::spatialToLinearAcceleration(robotStateD.dBodyVelocity, robotState.bodyVelocity).template cast<float>();
}
 ...
 */
template <typename T>
void ImuSimulator<T>::updateVectornav(const FBModelState<T> &robotState, const FBModelStateDerivative<T> &robotStateD,
                                      VectorNavData *data)
{
                                         ...
                                         const FBModelStateDerivative<T> &robotStateD, CheaterState<T> &state)
{
  ...
  state.acceleration = (R_body * Vec3<T>(0, 0, 9.81)) +
                       spatial::spatialToLinearAcceleration(robotStateD.dBodyVelocity, robotState.bodyVelocity);

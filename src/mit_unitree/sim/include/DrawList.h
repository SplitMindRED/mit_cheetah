...
public:
  ...
  void addBox(double depth, double width, double height, const Vec3<double> &pos, const Mat3<double> &ori,
              bool transparent);
  ...
  void updateRobotFromModel(DynamicsSimulator<T> &model, size_t id, bool updateOrigin = false)
  {
    ...
    {
      _kinematicXform.at(graphicsID) = spatialTransformToQT(model.getModel()._Xa.at(modelID));
    }
  ...
  {
    return _height_map_max;
  }
  const double &getHeightMapMin()
  {
    return _height_map_min;
  }
...
private:
  ...
  std::string _baseFileName = "/home/splitmind/itmo_ws/src/cheetah_ros/resources/";

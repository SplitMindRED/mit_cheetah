...
{
  printf("[DrawList] Load object files... from dir [%s]\n", _baseFileName.c_str());
  std::vector<std::string> names = { "c3_body.obj",   "mini_abad.obj", "c3_upper_link.obj",   "c3_lower_link.obj",
                                     "mini_body.obj", "mini_abad.obj", "mini_upper_link.obj", "mini_lower_link.obj",
                                     "sphere.obj",    "cube.obj" };
  for (const auto& name : names)
  {
    ...
    {
      setSolidColor(_colorData.back(), _vertexData.back().size(), disgustingGreen[0], disgustingGreen[1],
                    disgustingGreen[2]);
    }
    else
    {
      setSolidColor(_colorData.back(), _vertexData.back().size(), defaultRobotColor[0], defaultRobotColor[1],
                    defaultRobotColor[2]);
    }
...
void DrawList::addBox(double depth, double width, double height, const Vec3<double>& pos, const Mat3<double>& ori,
                      bool transparent)
{
  if (transparent)
  {
...
void DrawList::addMesh(double grid_size, const Vec3<double>& left_corner, const DMat<double>& height_map,
                       bool transparent)
{
  (void)transparent;

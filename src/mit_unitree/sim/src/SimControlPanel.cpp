...
std::string SimControlPanel::getDefaultUserParameterFileName()
{
  ...
  {
    throw std::runtime_error("default user parameter file has the wrong collection name, should be "
                             "user-parameter-file");
  }
...
void SimControlPanel::handleVelocityCMDLCM(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
void SimControlPanel::handleObstacleLCM(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
void SimControlPanel::handlePointsLCM(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
void SimControlPanel::handleIndexmapLCM(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
void SimControlPanel::handleHeightmapLCM(const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
{
  QString fileName =
      QFileDialog::getSaveFileName(nullptr, ("Save Simulator Table Values"), "../config", "All Files (*)");
...
{
  QString fileName =
      QFileDialog::getOpenFileName(nullptr, ("Load Simulator Table Values"), "../config", "All Files (*)");
  if (fileName == nullptr || fileName == "")
  {
    createErrorMessage("File name is invalid");
    return;
  ...
  {
    printf("new settings file %s doesn't contain the following simulator "
           "parameters:\n%s\n",
...
void SimControlPanel::on_robotTable_cellChanged(int row, int column)
{
  ...
  auto& parameter =
      (_simulationMode ? _simulation->getRobotParams() : _robotInterface->getParams()).collection.lookup(cellName);
  ControlParameterValueKind kind = parameter._kind;
  ControlParameterValue oldValue = parameter.get(kind);
  ...
  else
  {
    if (_simulationMode)
    {
      if (_simulation->isRobotConnected())
      {
        _simulation->sendControlParameter(cellName, parameter.get(parameter._kind), parameter._kind, false);
      }
    ...
    {
      _robotInterface->sendControlParameter(cellName, parameter.get(parameter._kind), parameter._kind, false);
    }
  }
}
...
{
  QString fileName =
      QFileDialog::getOpenFileName(nullptr, ("Load Quadruped Table Values"), "../config", "All Files (*)");
  if (fileName == nullptr || fileName == "")
  {
    createErrorMessage("File name is invalid");
    return;
  ...
  if (_simulationMode)
  {
    ...
    {
      printf("new settings file %s doesn't contain the following robot "
             "parameters:\n%s\n",
    ...
    if (_simulation->isRobotConnected())
    {
      ...
      {
        _simulation->sendControlParameter(kv.first, kv.second->get(kv.second->_kind), kv.second->_kind, false);
      }
    }
  ...
  else
  {
    ...
    {
      _robotInterface->sendControlParameter(kv.first, kv.second->get(kv.second->_kind), kv.second->_kind, false);
    }
...
void SimControlPanel::on_userControlTable_cellChanged(int row, int column)
{
  ...
  if (!success)
  {
    ...
    ui->userControlTable->item(row, 1)->setText(
        QString(_userParameters.collection.lookup(cellName).toString().c_str()));
    _ignoreTableCallbacks = false;
  }
  else
  {
    if (isRunning() || isError())
    {
      if (_simulationMode)
      {
        ...
        {
          _simulation->sendControlParameter(cellName, parameter.get(parameter._kind), parameter._kind, true);
        }

        _ignoreTableCallbacks = true;
        ui->userControlTable->item(row, 1)->setText(QString(parameter.toString().c_str()));
        _ignoreTableCallbacks = false;
      }
      else
      {
        _robotInterface->sendControlParameter(cellName, parameter.get(parameter._kind), parameter._kind, true);
      }
    }
  }
}
...
{
  QString fileName = QFileDialog::getOpenFileName(nullptr, ("Load User Table Values"), "../config", "All Files (*)");
  if (fileName == nullptr || fileName == "")
  {
    createErrorMessage("File name is invalid");
    return;
  ...
  _userParameters.defineAndInitializeFromYamlFile(fileName.toStdString());
  loadUserParameters(_userParameters);
  ...
  if (isRunning() || isError())
  {
    if (_simulationMode)
    {
      if (_simulation && _simulation->isRobotConnected())
      {
        ...
        {
          _simulation->sendControlParameter(kv.first, kv.second->get(kv.second->_kind), kv.second->_kind, true);
        }
      }
    }
    else
    {
      for (auto& kv : _userParameters.collection._map)
      {
        _robotInterface->sendControlParameter(kv.first, kv.second->get(kv.second->_kind), kv.second->_kind, true);
      }
    }
  }
}
...
{
  QString fileName = QFileDialog::getSaveFileName(nullptr, ("Save User Table Values"), "../config", "All Files (*)");
  if (fileName == nullptr || fileName == "")
  {
    createErrorMessage("File name is invalid");
    return;
...
void SimControlPanel::on_kickButton_clicked()
{
  ...
  kickVelocity << ui->kickAngularX->text().toDouble(), ui->kickAngularY->text().toDouble(),
      ui->kickAngularZ->text().toDouble(), ui->kickLinearX->text().toDouble(), ui->kickLinearY->text().toDouble(),
